...
export default function LogEntries({ user, entries, onRefresh }) {
  ...
  const handleDelete = async (id) => {
    ...
    if (isMockMode) {
      ...
      await supabase.from("log_entries").eq("id", id).delete();
    }
    onRefresh();
  };
  ...
  const handleSubmit = async (id) => {
    ...
    if (!entry?.activities?.trim()) {
      alert("Please add activities performed before submitting this daily log entry.");
      return;
    }

    if (isMockMode) {
      const e = MOCK_STATE.logEntries.find((item) => item.id === id);
      if (e) e.status = "submitted";
    } else {
      await supabase.from("log_entries").eq("id", id).update({ status: "submitted" });
    }
    onRefresh();
  };
    ...
    <div className="page animate-in">
      ...
      <div className="tabs">
        {["all", "draft", "submitted", "approved", "rejected"].map((s) => (
          ...
          >
            {s} {s !== "all" && `(${entries.filter((e) => e.status === s).length})`}
          </button>
        ))}
      </div>
          ...
          <table>
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id}>
                  <td>
                    <span className="font-mono text-sm">{fmt(e.entry_date)}</span>
                  </td>
                  ...
                  <td
                    style={{
                      ...
                      fontSize: 12,
                      color: "var(--text2)",
                      fontStyle: e.supervisor_comment ? "normal" : "italic",
                    }}
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      {e.status === "draft" && (
                        <>
                          ...
                          <button className="btn btn-primary btn-sm" onClick={() => handleSubmit(e.id)}>
                            Submit
                          </button>
                        </>
                      )}
