...
export default function SupervisorReview({ user, entries, reports, students, onRefresh }) {
  ...
  const reviewEntry = async (id, action) => {
    ...
    if (isMockMode) { const e = MOCK_STATE.logEntries.find(e => e.id === id); if (e) Object.assign(e, upd); }
    else await supabase.from("log_entries").eq("id", id).update(upd);
  ...
  const signReport = async (id) => {
    ...
    if (isMockMode) { const r = MOCK_STATE.weeklyReports.find(r => r.id === id); if (r) Object.assign(r, upd); }
    else await supabase.from("weekly_reports").eq("id", id).update(upd);
    setComment(""); setSelected(null); setLoading(false);
    onRefresh();
  };
